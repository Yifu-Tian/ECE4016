...
def checkRes(dcm, qnamelist):
    ...
    for i in qnamelist:
        ...
        if res.auth != []:
            ...
            ip_tmp = res.ar[0].rdata.__str__() if (res.ar != []) else query(res.auth[0].rdata, dcm)
        if res.auth == [] and res.rr != []:
            if (str(res.rr[0].rdata)[0].isdigit() == True): continue
            else:
                for RR in res.rr: dcm.add_answer(RR)
...
def local_DNS_Server(flag):
    ...
    while True:
        ...
        if message_parse.q.qname in cache:
            ...
            for RR in rr: DCmessage.add_answer(RR)
        else: 
            if flag == 0:
                request = message_parse.send("8.8.8.8")
                DCmessage = dnslib.DNSRecord.parse(request)
            else:
                DCmessage = message_parse.reply()
                res = query(message_parse.q.qname, DCmessage)
                for RR in res.rr: DCmessage.add_answer(RR)
